...
// Supports loading from SQLite database or legacy .releaselog.json files.
package main
...
// LoadIndexFromSQLite loads a SearchIndex from the SQLite database.
func LoadIndexFromSQLite(dbPath, assembly64Path string) (*SearchIndex, error) {
	slog.Info("Loading index from SQLite database", "path", dbPath)
	...
	rows, err := db.conn.Query(`
		SELECT id, title, group_name, year, file_type, category, path, primary_file,
			language, region, engine, top200_rank, is_4k,
			is_cracked, trainers, crack_flags, release_name,
			party, competition, is_onefile, rating, top500_rank, year_rank, party_rank
		FROM entries
	...
	foundCategories := map[string]bool{"All": true}

	for rows.Next() {
		var id int64
		var title, category, path string
		var groupName, year, fileType, primaryFile, language, region, engine, crackFlags, releaseName *string
		var party, competition *string
		var top200Rank, top500Rank, yearRank, partyRank *int
		var is4k, isCracked, isOnefile int
		var trainers int
		var rating *float64

		err := rows.Scan(
			&id, &title, &groupName, &year, &fileType, &category, &path, &primaryFile,
			&language, &region, &engine, &top200Rank, &is4k,
			&isCracked, &trainers, &crackFlags, &releaseName,
			&party, &competition, &isOnefile, &rating, &top500Rank, &yearRank, &partyRank,
		)
		if err != nil {
		...
		entry := ReleaseEntry{
			Name:         title,
			CategoryName: capitalizeFirst(category),
		}
		...
		if top200Rank != nil {
			entry.Top200Rank = *top200Rank
		}
		if top500Rank != nil {
			entry.Top500Rank = *top500Rank
		}
		if yearRank != nil {
			entry.YearRank = *yearRank
		}
		if partyRank != nil {
			entry.PartyRank = *partyRank
		}
		if releaseName != nil {
		...
		entry.Is4k = is4k != 0
		entry.IsOnefile = isOnefile != 0

		// Build crack info if cracked.
		if isCracked != 0 || trainers > 0 || (crackFlags != nil && *crackFlags != "") {
			entry.Crack = &CrackInfo{
				IsCracked: isCracked != 0,
				Trainers:  trainers,
			}
			if crackFlags != nil && *crackFlags != "" {
				entry.Crack.Flags = strings.Split(*crackFlags, ",")
			}
		}

		// Build full path.
		if primaryFile != nil && *primaryFile != "" {
			entry.FullPath = filepath.Join(assembly64Path, path, *primaryFile)
		} else {
			entry.FullPath = filepath.Join(assembly64Path, path)
		}
		entry.Path = path
		...
		if !foundCategories[catName] {
			...
			index.CategoryOrder = append(index.CategoryOrder, catName)
		}

		// Add entry and update ByCategory map.
	...
	slog.Info("Index loaded from SQLite", "total_entries", len(index.Entries), "categories", len(index.CategoryOrder)-1)
	return index, nil
}
